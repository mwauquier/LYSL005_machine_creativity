...
"""Complement for dealing with migration"""
...
class RNNTextGenerator:
    ...
    def __init__(
        ...
        with self._tf_graph.as_default():
            self._tf_sess = tf.compat.v1.Session()
            ...
            """
            self._tf_input = tf.compat.v1.placeholder(
                tf.float32,
                shape=(None, seq_length, vocab_size),
                name='inputs',
            )
            self._tf_target = tf.compat.v1.placeholder(
                tf.float32,
                shape=(None, seq_length, vocab_size),
                name='targets'
            )
            with tf.compat.v1.variable_scope(name):
                ...
                with tf.compat.v1.variable_scope('accuracy'):
                    self._tf_acc = tf.reduce_mean(input_tensor=tf.cast(
                        tf.equal(
                            tf.argmax(input=logits, axis=2),
                            tf.argmax(input=self._tf_target, axis=2),
                        ),
                        tf.float32,
                    ))
                self._tf_saver = tf.compat.v1.train.Saver()
                if logdir is not None:
                    self.logger = tf.compat.v1.summary.FileWriter(logdir, self._tf_graph)
            # Initialize the tf session
            self._tf_sess.run(tf.compat.v1.global_variables_initializer())
            self._tf_sess.run(tf.compat.v1.local_variables_initializer())
            self._params = {
